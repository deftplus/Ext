	...
	Пока НомерТекущейСтроки <= КоличествоСтрокВФайле Цикл
		...
		Иначе
			...
			Если Массив[0] = "Продажа_ШапкаДокумента" Тогда
				// глЭтоПрогрессивныйФормат = (СтрНайти(ИмяФайла, "РТУ_") > 0);
			...
			ИначеЕсли Массив[0] = "Поступление_ШапкаДокумента" Тогда
				//глЭтоПрогрессивныйФормат = (СтрНайти(ИмяФайла, "ПТУ_") > 0);
...
Функция МассивВСтруктуруДокумент(ЧастьДокумента, ВидДокумента, Массив, ИмяФайла = "")
	...
	#Область ПоступлениеТоваровУслуг
		Если ЧастьДокумента = "Поступление_ШапкаДокумента" Тогда
						Структура.Вставить("ВидОперации",				Перечисления.ВидыОперацийРеализацияТоваров.Товары);
						Структура.Вставить("ВидДокумента",				"ПоступлениеТоваровУслуг");
			х = х + 1;	Структура.Вставить("ОрганизацияИНН",			Массив[х] );
			//х = х + 1;	Структура.Вставить("ОбменПоЭДО",				Массив[х] );
						Структура.Вставить("ОбменПоЭДО",				Массив[х] );
			Если Структура.ОбменПоЭДО = "удаление" Тогда
				х = х + 1;	Структура.Вставить("Номер", 					Массив[х]);
				х = х + 1;	Структура.Вставить("КодРСДокумент",				Число(Массив[х]));
				х = х + 1;	Структура.Вставить("Дата", 						КонецДня(ТекущаяДата()) ); // время 09.00, Карпова Наталья 18.07.2022
				...
				ДокПоступлениеТоваровУслугСсылка = НайтиДокументПоКодуРС(Структура);
				Если ДокПоступлениеТоваровУслугСсылка = Неопределено Тогда
					ЗарегистрироватьИзменение("Док реализация: " + СокрЛП(Структура.Номер) + "-" + СокрЛП(Структура.КодРСДокумент) + ", от " + Структура.Дата, Перечисления.СобытияЗагрузки.Изменен, ТекущаяДата(), , "Документ.РеализацияТоваровУслуг");
				Иначе
					// проверим на дату запрета редактирования
					Если ДокПоступлениеТоваровУслугСсылка.Дата <= ДатаЗапретаРедактированияДляПользователя Тогда
						СтруктураОшибок.Вставить("ТипОшибки", "ОшибкаПоДатеЗапрета");
						ЗарегистрироватьИзменение(ДокПоступлениеТоваровУслугСсылка, Перечисления.СобытияЗагрузки.ДатаЗапретаУдаление, ТекущаяДата(), ДокПоступлениеТоваровУслугСсылка.Дата, "Документ.ПоступлениеТоваровУслуг");
						ТекстОшибки = "Документ в закрытом периоде. Перемещен в ошибки по дате.";
						ЭлементыПути = СтрРазделить(ИмяФайла, "\", Ложь);
						ЗарегистрироватьОшибку( Новый Структура("ТекОб, ИмяФайла, ТекущаяДата, ОписаниеОшибки, ID_Документа, Номер", ДокПоступлениеТоваровУслугСсылка, ЭлементыПути[ЭлементыПути.ВГраница()], ТекущаяДата(), ТекстОшибки, Структура.КодРС, "") );
						Возврат Неопределено;
					Иначе
						ЗарегистрироватьИзменение(ДокПоступлениеТоваровУслугСсылка, Перечисления.СобытияЗагрузки.Изменен, ТекущаяДата(), ДокПоступлениеТоваровУслугСсылка.Дата, "Документ.ПоступлениеТоваровУслуг");
					КонецЕсли;
				КонецЕсли;
			...
			Иначе
				...
				х = х + 1;	Структура.Вставить("КодРСДокумент",				Число(Массив[х]) );
				х = х + 1;	Структура.Вставить("Склад",						Справочники.Склады.НайтиПоКоду("00-000001") );
				...
				//х = х + 1;	Структура.Вставить("СуммаВключаетНДС",			Массив[х]);
							Структура.Вставить("ВалютаДокумента",			Справочники.Склады.НайтиПоКоду("643") );
				...
				//х = х + 1;	Структура.Вставить("ГрузополучательКодРС",		Число(Массив[х]));
							Структура.Вставить("Номер", 					 Структура.НомерВходящегоДокумента );
							Структура.Вставить("Статус",						"НовыйИзменение" );
			КонецЕсли;
			...
			Структура.Вставить("Ответственный",				Ответственный);
			Структура.Вставить("Комментарий",				СтрШаблон("Контрагент: %1, склад: %2.", Структура.КонтрагентКодРС, КомментарийШаблон));
		...
		ИначеЕсли ЧастьДокумента = "Поступление_ТЧ_Документа" Тогда
			...
			х = х + 1;	Структура.Вставить("СтранаПроисхожденияКод",	Массив[х] );
						...
						Структура.Вставить("СтавкаНДС",					Перечисления.СтавкиНДС.НДС20 );
						Структура.Вставить("СуммаНДС",					Окр( Структура.Всего / 120 * 20, 2, 1 ) );
						Структура.Вставить("Сумма",						Структура.Всего - Структура.СуммаНДС );
						Структура.Вставить("Цена",						Окр( Структура.Сумма / Структура.Количество, 2, 1 ) );
...
Процедура ОбработатьШапкуДокумента(ВидДокумента, Структура)
	...
	ИначеЕсли ВидДокумента = "ПоступлениеТоваровУслуг" Тогда
		...
		ИначеЕсли НЕ ДокПоступлениеТоваровУслугСсылка = Неопределено Тогда
			ДокПоступлениеТоваровУслуг = ДокПоступлениеТоваровУслугСсылка.ПолучитьОбъект();
			Если Структура.Статус = "удаление" Тогда
				ДокПоступлениеТоваровУслуг.ПометкаУдаления = Истина;
			Иначе
				ДокПоступлениеТоваровУслуг.Товары.Очистить();
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ Структура.Статус = "удаление" Тогда
			...
			Если ЗначениеЗаполнено(Структура.КодРСДокумент) Тогда
				ДобавитьДополнительноеСвойство( ДокПоступлениеТоваровУслуг.Ссылка, "КодРСДокументы", Структура.КодРСДокумент )
			КонецЕсли;
		КонецЕсли;
...
ДатаПоследнегоИзменения = '2022.08.10';
